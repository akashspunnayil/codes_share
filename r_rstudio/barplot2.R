...
a<- ggplot(data.m,aes(x=Regions, y = value)) + 
  facet_wrap(~variable) +
  geom_bar(aes(fill = factor(Regions)),position="dodge", stat = "identity", width = 0.3)
plot(a)
b<- a+theme_gray()+theme(text=element_text(size=20, family="TT Times New Roman"))
c<- b+labs(title="", x ="Regions", y = "CF area (%)",legend.title ="" )
plot(c)

# in inches
tiff("CF_percent.tiff", width = 12, height = 8, units = 'in', res = 300)
plot(c)
dev.off()
getwd()
